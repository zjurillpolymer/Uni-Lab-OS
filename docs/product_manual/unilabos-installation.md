...
> **推荐阅读顺序：** 先了解工作区和设备包组成结构，用初始化命令创建最小骨架，再根据设备控制方式选择接入模板；随后定义物料、编写启动图（Graph JSON），最后完成实验操作（子工作流）和完整工作流。
...
命令创建 15 个文件，包含一个示例设备、一个回显动作和一个普通工作流，以及启动图、配置和测试。物料和实验操作目录预留为空，后续按实际业务补充。完整参数、文件清单和检查命令见[初始化工作区](workspace.md#初始化工作区)。
