...
适用环境：通用安装与接入场景　·　手册版本：2026.09.10　·　设备包可在安装 OS 后创建
</div>
...
- **编写设备包**：先用[初始化工作区](workspace.md#初始化工作区)中的 `unilab workspace init` 创建设备包骨架，再按[设备包规范与系统启动](unilabos-installation.md)补充设备、物料、启动图和工作流。
